...
func (s *CurrencyServer) Convert(ctx context.Context, req *gen.ConvertRequest) (*gen.ConvertResponse, error) {
...
func (s *CurrencyServer) ConvertBatch(ctx context.Context, req *gen.ConvertBatchRequest) (*gen.ConvertBatchResponse, error) {
	UserID, ok := interceptors.GetUserID(ctx)
	if !ok {
	...
	return &gen.ConvertBatchResponse{
		From:      req.From,
		Results:   result,
		Amount:    req.Amount,
